...
function AuthProviderCore({ children }: { children: ReactNode }) {
  ...
  // Enhanced profile creation with native app support and timeout handling
  const ensureProfileExists = async (forceRetry = false): Promise<boolean> => {
    const currentUser = user || (await supabase.auth.getUser()).data.user;
    if (!currentUser) {
      console.log('❌ [AuthContext] No user available for profile creation');
      return false;
    }
    ...
    try {
      console.log('🔧 [AuthContext] Starting enhanced profile creation process...');
      
      // Create a timeout promise for native apps that might get stuck
      const timeoutPromise = new Promise<boolean>((_, reject) => {
        setTimeout(() => {
          reject(new Error('Profile creation timeout (30s)'));
        }, 30000);
      });

      // Profile creation promise with retries
      const profileCreationPromise = async (): Promise<boolean> => {
        const maxRetries = 3;
        let attempt = 0;
        
        while (attempt < maxRetries) {
          attempt++;
          console.log(`🔄 [AuthContext] Profile creation attempt ${attempt}/${maxRetries}`);
          
          try {
            const success = await ensureProfileExistsService(currentUser);
            ...
            if (attempt < maxRetries) {
              console.log(`⏳ [AuthContext] Attempt ${attempt} failed, waiting before retry...`);
              await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
            }
            
          } catch (error) {
            console.error(`❌ [AuthContext] Attempt ${attempt} failed with error:`, error);
        ...
        console.error('❌ [AuthContext] All profile creation attempts failed');
        return false;
      };

      // Race between profile creation and timeout
      const success = await Promise.race([profileCreationPromise(), timeoutPromise]);
      
      return success;
      
    } catch (error) {
      console.error('❌ [AuthContext] Profile creation failed with error:', error);
      
      if (error instanceof Error && error.message.includes('timeout')) {
        console.error('⏰ [AuthContext] Profile creation timed out');
        // For native apps, we might want to show a different message
