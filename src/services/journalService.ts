 ...
 */
export const createPlaceholderEntry = async (userId: string): Promise<boolean> => {
  try {
    console.log('[JournalService] Creating placeholder entry for new user:', userId);
    
    const placeholderContent = `Welcome to SOULo Journal! This is an example entry to help you get started.

Your entries will appear like this. Try recording your thoughts by using the "Record" tab above.

This placeholder entry includes sample themes and emotions to show you how your real entries will look after our AI analyzes them.`;
    
    // Sample themes
    const sampleThemes = ["getting started", "reflection", "welcome"];
    
    // Sample emotions with intensities
    ...
    const { error } = await supabase
      ...
      .insert([{
        user_id: userId,
        "refined text": placeholderContent,
        "transcription text": placeholderContent,
        created_at: new Date().toISOString(),
        master_themes: sampleThemes,
        sentiment: "positive",
        emotions: sampleEmotions,
        duration: 0, // This is a placeholder, not a real recording
        Edit_Status: 0,
      }]);
    ...
    console.log('[JournalService] Placeholder entry created successfully');
    return true;
  } catch (error: any) {
    console.error('[JournalService] Error creating placeholder entry:', error.message);
    return false;
