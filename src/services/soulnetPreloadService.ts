...
export class SoulNetPreloadService {
  ...
  private static generateGraph(entityEmotionMap: Record<string, Record<string, number>>): { nodes: NodeData[], links: LinkData[] } {
    ...
    console.log("[SoulNetPreloadService] UPDATED Y-AXIS: Generating graph with", entityList.length, "entities with emotions distributed vertically on Y-axis (-40 to +40)");
    ...
    console.log("[SoulNetPreloadService] UPDATED Y-AXIS: Generated graph with", nodes.length, "nodes and", links.length, "links");
    console.log("[SoulNetPreloadService] UPDATED Y-AXIS: Entities form circle in center layer, emotions distributed vertically on Y-axis from -40 to +40");
    return { nodes, links };
  }
