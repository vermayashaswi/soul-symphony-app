...
export const supabase = createClient<Database>(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
  global: {
    fetch: (url, options) => {
      ...
      // Debug logging for table name issues
      const urlStr = url.toString();
      if (urlStr.includes('Journal_Entries') || urlStr.includes('journal_entries') || urlStr.includes('Journal%20Entries')) {
        console.log('DEBUG - Table access detected:', urlStr);
      }
      ...
      .then(response => {
        ...
        // Debug logging for responses about Journal_Entries table
        if (urlStr.includes('Journal_Entries') || urlStr.includes('journal_entries') || urlStr.includes('Journal%20Entries')) {
          response.clone().text().then(text => {
            try {
              console.log('DEBUG - Journal table response:', text.substring(0, 200) + '...');
            } catch (err) {
              console.log('DEBUG - Could not log response body');
            }
          }).catch(() => {});
        }
