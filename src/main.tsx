...
const fixViewportHeight = () => {
  console.log('[ViewportFix] Initializing viewport height fix');
  
  // Set CSS variable for viewport height that updates on resize
  const setVhProperty = () => {
    const vh = window.innerHeight * 0.01;
    document.documentElement.style.setProperty('--vh', `${vh}px`);
    console.log('[ViewportFix] Set viewport height:', vh);
  };
  ...
  if (/iPad|iPhone|iPod/.test(navigator.userAgent) || 
    ...
    console.log('[ViewportFix] iOS device detected, setting up keyboard handling');
...
const initializeApp = async () => {
  ...
  try {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent) || 
      ...
      console.log('[App] iOS device detected');
    }
...
console.log('[App] React application rendered');
