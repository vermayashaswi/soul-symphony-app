...
const MobileChatMessage: React.FC<MobileChatMessageProps> = ({ message, showAnalysis }) => {
  ...
  return (
    <div className={`relative flex items-start gap-2 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
      ...
      <div
        className={cn(
          "min-w-0 max-w-[85%] rounded-2xl p-3 text-sm",
          message.role === 'user' 
      ...
      >
        ...
        {hasReferences && (
          <div className="mt-2">
            ...
            {showReferences && (
              <div className="mt-1 text-xs max-h-24 overflow-y-auto border-l-2 border-primary/30 pl-2 pr-1">
