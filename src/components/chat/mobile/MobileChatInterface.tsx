  ...
  const {
    ...
    queryCategory
     ...
     onFinalResponse: async (response) => {
       ...
       debugLog.addEvent("Streaming Response", `[Mobile] Final response received for ${threadId}: ${responseText.substring(0, 100)}...`, "success");

         // CRITICAL: Immediate UI update for first messages to prevent blank screen
         setMessages(prev => {
           // Check if this is a duplicate
           const lastMsg = prev[prev.length - 1];
           if (lastMsg && lastMsg.role === 'assistant' && lastMsg.content === responseText) {
         ...
         debugLog.addEvent("Streaming Response", `[Mobile] Assistant message added to UI for thread ${threadId}`, "success");
            ...
            if (recentUserMessages && recentUserMessages.length > 0) {
              await updateUserMessageClassification(
                ...
                { 
                  ...
                  confidence: 0.9,
                  reasoning: 'Auto-classified during mobile chat',
                  useAllEntries: false
                },
                user.id
              );
         ...
         try {
           setTimeout(async () => {
             ...
             if (!currentThreadIdRef.current || currentThreadIdRef.current !== threadId) return;
             ...
             if (!alreadyExists) {
                ...
                try {
                  const correlationId = `assistant-${threadId}-${Date.now()}`;
