              ...
              <div className="flex items-center gap-1">
                {!sidebarCollapsed && (
                  <Button
                    ...
                    className={`h-8 w-8 text-muted-foreground hover:text-foreground ${isProcessingActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={isProcessingActive}
                    title={isProcessingActive ? "Please wait while processing..." : "New conversation"}
                  >
                ...
                <Button
                  ...
                  className={`h-8 w-8 text-muted-foreground hover:text-foreground ${isProcessingActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                  disabled={isProcessingActive}
                  title={isProcessingActive ? "Please wait while processing..." : (sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar")}
                >
                  {sidebarCollapsed ? (
              ...
              {sidebarCollapsed ? (
                <div className="p-2 space-y-2">
                  <Button
                    ...
                    size="icon"
                    onClick={handleCreateNewThread}
                    className={`w-full h-10 ${isProcessingActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={isProcessingActive}
                    title={isProcessingActive ? "Please wait while processing..." : "New conversation"}
                  >
                    <Plus className="h-4 w-4" />
