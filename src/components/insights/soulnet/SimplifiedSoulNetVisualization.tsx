...
export const SimplifiedSoulNetVisualization: React.FC<SimplifiedSoulNetVisualizationProps> = ({
  ...
  const maxEdgeThickness = useMemo(() => {
    if (!data.links.length) return 1;
    return Math.max(...data.links.map(link => link.value));
  }, [data.links]);

  const shouldShowNodeLabel = (nodeId: string) => {
  ...
  const isEdgeDimmed = (link: LinkData) => {
    if (!selectedNode) return false;
        ...
        {data.links.map((link, index) => {
          ...
          return (
            <Edge
              ...
              startNodeScale={sourceNode.value}
              endNodeScale={targetNode.value}
            />
          );
        })}
