...
serve(async (req) => {
  ...
  try {
    ...
    if (isEmotionQuery && uniqueResults.some(r => r.source === 'sql_query' && r.query_function === 'get_top_emotions_with_entries')) {
      // Format emotion data specifically for emotion queries
      const emotionResults = uniqueResults.filter(r => r.source === 'sql_query' && r.query_function === 'get_top_emotions_with_entries');
      
      journalContext = "**EMOTION ANALYSIS DATA (from database emotion scores):**\n\n";
      
      emotionResults.forEach((result, index) => {
        journalContext += `**${index + 1}. ${result.emotion.toUpperCase()}** (Score: ${result.score})\n`;
        
        if (result.sample_entries && Array.isArray(result.sample_entries)) {
          journalContext += "Sample entries with this emotion:\n";
          result.sample_entries.forEach((entry, entryIndex) => {
            const date = new Date(entry.created_at).toLocaleDateString();
            journalContext += `  - ${date}: "${entry.content}" (Score: ${entry.score})\n`;
          });
        }
        journalContext += "\n";
      });
      
      // Add any vector search results as supporting content
      const vectorResults = uniqueResults.filter(r => r.source === 'vector_search');
      if (vectorResults.length > 0) {
        journalContext += "\n**SUPPORTING JOURNAL ENTRIES:**\n\n";
        vectorResults.slice(0, 3).forEach(entry => {
          const date = new Date(entry.created_at).toLocaleDateString();
          journalContext += `- ${date}: ${entry.content.substring(0, 200)}...\n`;
        });
      }
    } else {
...
${isEmotionQuery ? `EMOTION ANALYSIS INSTRUCTIONS:
- You have access to PRE-CALCULATED emotion scores from the database (0.0 to 1.0 scale)
- These scores were generated by AI analysis of the journal content
- Focus on the quantitative emotion data provided, NOT on inferring emotions from text
- The emotion scores represent the strength/intensity of each emotion in the entries
- Analyze patterns, frequencies, and relationships between emotions based on the scores
- Do NOT say things like "your entry doesn't explicitly mention emotions" - the emotions are already calculated and stored` : ''}
