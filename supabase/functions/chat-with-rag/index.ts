...
// Generate embeddings using the create-embedding function
async function generateEmbedding(text: string): Promise<number[]> {
  ...
  try {
    console.log("Calling create-embedding function for query:", text.substring(0, 50) + "...");
    
    // Call the existing create-embedding function instead of generate-embedding
    const response = await supabase.functions.invoke('create-embedding', {
      body: { text },
    });

    if (response.error) {
      console.error('Error from create-embedding function:', response.error);
      functionResult.executionTime = Date.now() - startTime;
      functionResult.result = { error: response.error };
      throw new Error(`Failed to generate embedding: ${response.error.message || 'Unknown error'}`);
    }

    if (!response.data || !response.data.embedding) {
      console.error('Invalid response from create-embedding function:', response);
      functionResult.executionTime = Date.now() - startTime;
      functionResult.result = { error: 'Invalid response structure' };
    ...
    console.log(`Successfully generated embedding with ${response.data.embedding.length} dimensions`);
