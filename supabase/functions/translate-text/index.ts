...
serve(async (req) => {
  ...
  try {
    ...
    const logPrefix = entryId ? `[Entry ${entryId}]` : '[Static text]';
    console.log(`${logPrefix} Translating text: "${text.substring(0, 50)}..." from ${sourceLanguage || 'auto-detect'} to ${targetLanguage}`);
    ...
    if (!sourceLanguage) {
      ...
      console.log(`${logPrefix} Detecting language with URL: ${detectUrl}`);
      ...
      if (!detectResponse.ok) {
        ...
        console.error(`${logPrefix} Language detection failed with status ${detectResponse.status}: ${errorData}`);
        throw new Error(`Language detection failed: ${detectResponse.statusText}`);
      }

      const detectData = await detectResponse.json();
      console.log(`${logPrefix} Detect API response:`, JSON.stringify(detectData));
      
      if (!detectData.data || !detectData.data.detections || !detectData.data.detections[0] || !detectData.data.detections[0][0]) {
        console.error(`${logPrefix} Invalid detection response format:`, JSON.stringify(detectData));
    ...
    console.log(`${logPrefix} Sending translation request to: ${translateUrl}`);
    console.log(`${logPrefix} Request parameters: from ${detectedLanguage} to ${targetLanguage}`);
    ...
    if (!translateResponse.ok) {
      ...
      console.error(`${logPrefix} Translation failed with status ${translateResponse.status}: ${errorData}`);
      throw new Error(`Translation failed: ${translateResponse.statusText}`);
    }

    const translateData = await translateResponse.json();
    console.log(`${logPrefix} Translation API response:`, JSON.stringify(translateData));
    
    // Check if the response has the expected structure
    if (!translateData.data || !translateData.data.translations || !translateData.data.translations[0]) {
      console.error(`${logPrefix} Invalid translation response format:`, JSON.stringify(translateData));
      throw new Error('Invalid translation response format');
    }
    
    const translatedText = translateData.data.translations[0].translatedText;
    console.log(`${logPrefix} Translated text: "${translatedText.substring(0, 50)}..."`);

    // Update the database with the translation if entryId is provided
    if (entryId) {
      ...
      // Trigger post-processing functions if necessary
      try {
        if (entryId) {
