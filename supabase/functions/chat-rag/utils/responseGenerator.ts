...
// Define the conversational SOuLO prompt for journal-specific queries with dynamic schema
const JOURNAL_SPECIFIC_PROMPT_TEMPLATE = `You are SOULo, an AI mental health therapist assistant that helps users understand their journal entries and emotional patterns through evidence-based therapeutic analysis.
...
AVAILABLE DYNAMIC SCHEMA:
- Emotions ({emotionCount}): {availableEmotions}
- Themes ({themeCount}): {availableThemes}
...
• Reference only the available emotions: {availableEmotions}

THEME ANALYSIS INSTRUCTIONS:
• Use only the available theme categories: {availableThemes}
...
/**
 * Generate a response using the entries and user message with dynamic schema
 */
export async function generateResponse(
  ...
  try {
    // Use provided dynamic lists or fallback to defaults
    const availableEmotions = dynamicEmotions || defaultEmotions;
    const availableThemes = dynamicThemes || allowedCategories;
    ...
    const entriesWithDates = entries.map(entry => {
      ...
      if (entry.emotions && typeof entry.emotions === 'object') {
        const emotions = Object.entries(entry.emotions)
          .filter(([emotion, score]) => {
            // Only include emotions that are in our available list
            return typeof score === 'number' && score > 0.3 && availableEmotions.includes(emotion);
          })
      ...
      // Format themes info (only include available themes)
      let themeInfo = '';
      if (entry.master_themes && Array.isArray(entry.master_themes)) {
        const validThemes = entry.master_themes.filter(theme => availableThemes.includes(theme));
        if (validThemes.length > 0) {
          themeInfo = `\nThemes: ${validThemes.join(', ')}`;
        }
      }
    ...
    // Prepare prompt with dynamic schema information
    const promptFormatted = JOURNAL_SPECIFIC_PROMPT_TEMPLATE
      .replace('{journalData}', entriesWithDates)
      .replace('{userMessage}', message)
      .replace('{startDate}', startDateFormatted)
      .replace('{endDate}', endDateFormatted)
      .replace('{emotionCount}', availableEmotions.length.toString())
      .replace('{availableEmotions}', availableEmotions.join(', '))
      .replace('{themeCount}', availableThemes.length.toString())
      .replace('{availableThemes}', availableThemes.join(', '));
      
    // Call OpenAI with dynamic schema-enhanced prompt
    console.log("Calling OpenAI with dynamic schema-enhanced SOuLO prompt format");
    ...
    console.log('Generated response with dynamic schema integration');
