-- Enhanced execute_dynamic_query function with time variable support and better error handling
...
BEGIN
  ...
  -- Replace time variables with actual timestamps
  ...
  processed_query := REPLACE(processed_query, '__CURRENT_WEEK_START__', '''' || (date_trunc('week', current_time))::text || '''');
  processed_query := REPLACE(processed_query, '__CURRENT_WEEK_END__', '''' || (date_trunc('week', current_time) + INTERVAL '6 days 23:59:59')::text || '''');
  processed_query := REPLACE(processed_query, '__LAST_WEEK_START__', '''' || (date_trunc('week', current_time) - INTERVAL '7 days')::text || '''');
  processed_query := REPLACE(processed_query, '__LAST_WEEK_END__', '''' || (date_trunc('week', current_time) - INTERVAL '1 second')::text || '''');
  processed_query := REPLACE(processed_query, '__CURRENT_MONTH_START__', '''' || (date_trunc('month', current_time))::text || '''');
  processed_query := REPLACE(processed_query, '__CURRENT_MONTH_END__', '''' || (date_trunc('month', current_time) + INTERVAL '1 month - 1 second')::text || '''');
  processed_query := REPLACE(processed_query, '__LAST_MONTH_START__', '''' || (date_trunc('month', current_time) - INTERVAL '1 month')::text || '''');
  processed_query := REPLACE(processed_query, '__LAST_MONTH_END__', '''' || (date_trunc('month', current_time) - INTERVAL '1 second')::text || '''');
