-- Drop the existing table and recreate with the proper structure
DROP TABLE IF EXISTS public.api_usage CASCADE;

-- Create api_usage table with the correct structure
...
CREATE INDEX idx_api_usage_ip_created ON public.api_usage(ip_address, created_at);
CREATE INDEX idx_api_usage_function_created ON public.api_usage(function_name, created_at);
