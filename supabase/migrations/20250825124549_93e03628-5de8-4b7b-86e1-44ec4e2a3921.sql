-- Final working version of schedule_journal_reminders function
...
BEGIN
  ...
  WHERE notification_type = 'journal_reminder' 
    AND status = 'pending' 
    AND scheduled_for < (current_ts + INTERVAL '24 hours');
  ...
  LOOP
    ...
    IF jsonb_typeof(user_record.reminder_settings->'reminders') = 'array' THEN
      ...
      LOOP
        ...
        IF (reminder_record.reminder_data->>'enabled')::boolean = true THEN
          ...
          -- Create the timestamp in a simple way that works
          scheduled_time := (
            CURRENT_DATE + (reminder_record.reminder_data->>'time')::TIME
          )::TIMESTAMPTZ;
          
          -- Convert to user timezone then to UTC
          scheduled_time := timezone('UTC', timezone(user_timezone_offset, scheduled_time));
          
          -- If time has passed today, schedule for tomorrow
          IF scheduled_time < current_ts THEN
    ...
    ELSE
      ...
      IF (user_record.reminder_settings->>'enabled')::boolean = true THEN
        ...
        user_timezone_offset := COALESCE(user_record.timezone, 'UTC');
        scheduled_time := (CURRENT_DATE + '19:00:00'::TIME)::TIMESTAMPTZ;
        scheduled_time := timezone('UTC', timezone(user_timezone_offset, scheduled_time));
