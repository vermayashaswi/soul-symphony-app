-- Fix the schedule_journal_reminders function to handle the timestamp comparison error
  ...
  LOOP
    ...
    IF jsonb_typeof(user_record.reminder_settings->'reminders') = 'array' THEN
      ...
      LOOP
        ...
        IF (reminder_record.reminder_data->>'enabled')::boolean = true THEN
          ...
          -- Fixed: Cast the time string to time type before combining with date
          scheduled_time := (CURRENT_DATE + (reminder_record.reminder_data->>'time')::time) 
    ...
    ELSE
      ...
      IF (user_record.reminder_settings->>'enabled')::boolean = true THEN
        ...
        user_timezone_offset := COALESCE(user_record.timezone, 'UTC');
        scheduled_time := (CURRENT_DATE + '19:00:00'::time) 
                         AT TIME ZONE user_timezone_offset 
                         AT TIME ZONE 'UTC';
